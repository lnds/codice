...
from developers.charts import get_devs_blame_data, get_devs_owner_pie_chart, get_devs_churn_production_chart, \
    get_devs_quadrant_chart, get_dev_activity_chart
from developers.models import Developer, Blame
from developers.services import get_developers_blame_summaries, get_developer_blame_summary, get_developer_commits, \
    change_alias_of
...
from repos.punchcards import get_dev_punchcard
...
class DeveloperList(DeveloperMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        blames = self.blames[(page_obj.number - 1) * self.paginate_by:(page_obj.number * self.paginate_by)]
        devs = []
        for blame in blames:
            devs.append(Developer.objects.get(pk=blame['author']))
...
class DeveloperUpdate(DeveloperMixin, UpdateView):
    fields = ['description', 'enabled']
    success_message = _('Developer was updated successfully')
    template_name = 'author/author_form.html'

    def form_valid(self, form):
        result = super().form_valid(form)
        developer = self.object
        if 'is_alias_of' in self.request.POST:
            alias_id = self.request.POST['is_alias_of']
            try:
                alias_of = Developer.objects.get(pk=alias_id)
                developer = change_alias_of(developer, alias_of)
            except Exception as e:
                developer = change_alias_of(developer, None)
        else:
            developer = change_alias_of(developer, None)
        return result
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['developer'] = self.object
        return context

    def get_success_url(self):
        developer = self.object
        id = developer.is_alias_of.id if developer.is_alias_of else developer.id
        return reverse_lazy('full-developer-profile', kwargs={'pk': id})
